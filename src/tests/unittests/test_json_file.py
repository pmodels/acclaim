...
from src.json_file.json_file import ReadJsonError, read_generic_json_file, read_generic_json_file_ch4, read_collective_shell, get_selections, get_rules, rules_to_dict, shell_wrapper, update_collective
...
class TestJson(unittest.TestCase):
  ...
  def test_shell_wrapper_ch4(self):
    ...
    rules_dict = shell_wrapper(collective, rules_to_dict(collective, rules, algs))
    result = list(rules_dict["comm_hierarchy=parent"]["is_commutative=yes"]["comm_size=any"]["comm_avg_ppn=any"]["avg_msg_size=any"].keys())
    self.assertEqual(result[0], "composition=MPIDI_allreduce_intra_composition_alpha")
  ...
  def test_update_collective(self):
    ...
    answer = False
    to_check = list(json_file_data["collective=bcast"]["comm_type=intra"]["comm_size=any"]["comm_avg_ppn=any"]["avg_msg_size=any"].keys())
  ...
  def test_update_collective_ch4(self):
    ...
    answer = False
    to_check = list(json_file_data["collective=allreduce"]["comm_type=intra"]["comm_hierarchy=parent"]["is_commutative=yes"]["comm_size=any"]["comm_avg_ppn=any"]["avg_msg_size=any"].keys())
